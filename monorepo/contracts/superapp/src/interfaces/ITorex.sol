...
interface ITorexController {
    ...
    function onLiquidityMoved(
        LiquidityMoveResult calldata result
...
interface ITorex {
    ...
    function getPairedTokens()
        external
        view
        returns (ISuperToken inToken, ISuperToken outToken);
    ...
    function estimateApprovalRequired(
        int96 flowRate
    ) external view returns (uint256 requiredApproval);

    /// Let liquidity mover (contracts with ILiquidityMover interface) trigger a liquidity movement.
    function moveLiquidity(
        bytes calldata moverData
    ) external returns (LiquidityMoveResult memory result);
}
